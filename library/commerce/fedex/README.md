...
- The four read-only MCP tools use operation-specific nested request schemas plus server-side validation before any HTTP call. Rate requests preserve official optional rate types, controls, carrier choices, and grouped-package semantics rather than imposing single-package shipment-validation limits. Address validation requires FedEx's common minimum (`streetLines` and `countryCode`), applies the documented US alternatives (`postalCode` or `city` plus `stateOrProvinceCode`), and models resolution controls. Shipment validation enforces required pickup, total-weight, payment/conditional-payor, label, party, and one-package details and requires explicit FedEx success/failure alerts. Pickup availability validates official nested shipment/package details through the same validator used by scheduling preflight, preserves each `output.options` entry (carrier, availability, date, cutoff, and structured access duration), and drops addresses, messages, request echoes, and unrelated response fields.
